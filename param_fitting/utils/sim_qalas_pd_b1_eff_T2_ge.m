function [Mz_mtx, Mxy_mtx] = sim_qalas_pd_b1_eff_T2_ge(TR, alpha_deg, esp, turbo_factor, t1_vals, t2_vals, num_reps, echo2use, heartbeat, time2relax_at_the_end, b1, inv_eff, pd_map )
%
% TR parameter : excludes dead time at the end of each TR ( total_TR is TR + time2relax_at_the_end )

% we need these:
%   heartbeat               (can potentially be in the private field)
%   turbo factor
% from the dicom header
% we already have inner TR (esp) as "Repetition Time"
...
%    gap_between_readouts = 900e-3;
    heartbeat = 66.67;  % read this from the advanced tab
                        % or perhaps from the dicom header?
end

gap_between_readouts = 60 / heartbeat;  % definition in GE


% esp -> can be read from GE dicom header as "Repetition Time" 

% THERE IS ALWAYS A 160 UM GAP AT THE END OF EACH BLOCK IN THE GE SEQUENCE
% THIS GAP IS INCLUDED IN THE REPETITION TIME READ FROM THE DICOM HEADER

% AND THERE ARE ALWAYS 2 READOUTS IN EACH ETL WHICH ARE DUMMIES
echo2use = 3;       % USE THE 3RD ECHO AS THE DC POINT
...
etl = turbo_factor * esp;   % THIS NEEDS TO INCLUDE THE TWO DUMMIES (e.g. etl=130 for actual 128 turbo factor in the advanced tab)
...
delT_M1_M2 = 92.8e-3;                % duration of T2 prep

crusher_after_T2prep = 2.34e-3;
...
delT_M4_M5 = 16.2e-3;               % duration of the inversion pulse (excluding any gradients after)


%delT_M5_M6 = 100e-3 - 6.45e-3;       % gap between end of inversion and start of readout#2
delT_M5_M6 = 97.34e-3 + 160e-6 - delT_M4_M5/2;  % assuming inversion happens at the middle of the adibatic inv pulse
...
for reps = 1:num_reps
    ...
    M2 = relax_T2_b1(M1, delT_M1_M2 - crusher_after_T2prep, t2_vals, b1, t1_vals);
    ...
    for q = 1:tf
        ...
        time = esp;     % (now includes the 160us gap in each block)
    end
