...
% data_file_path = '/autofs/space/marduk_002/users/shohei/2025_05_28_bay4_mwf_repository/meas_MID00164_FID134571_tAFI_hbcd.dat';
% traj            = readmatrix('utils/fov_240x208_msize_60x52_tl_4_ncal_0_spiral_4_acc_1.5x1.5_nTR284_nph1.txt');

% example data available at https://zenodo.org/records/15546728
load('kspace_unsorted_tAFI_hbcd.mat')

traj            = readmatrix('utils/fov_228x176_msize_56x44_tl_4_ncal_0_spiral_4_acc_1.5x1.5_nTR237_nph1.txt');
...
pulseq_file_path            =       'REPOSITORY/sequences/mwf_qalas_hbcd_tAFI/tAFI_hbcd.seq';
seq                         =       mr.Sequence();
...
imagesc3d2(S1 , s(S1)/2+[0 0 0], 1, 180+[0,0,0], [0,3e-4],1,'S1'), setGcf(.5)
imagesc3d2(S2 , s(S2)/2+[0 0 0], 2, 180+[0,0,0], [0,3e-4],1,'S2'), setGcf(.5)
...
imagesc3d2( rsos(img_patref_pad,4), s(img_patref_pad)/2, 10, 180+[0,0,0], [0,7e-3],1,'img patref pad'), setGcf(.5)
imagesc3d2( rsos(ifft3call(kspace_svd(:,:,:,:,end)),4), s(img_patref_pad)/2, 11, 180+[0,0,0], [0,7e-3],1,'kspace svd ifft'), setGcf(.5)
...
imagesc3d2(S1 , s(S1)/2+[0 0 0], 1, 180+[0,0,0], [0,7e-3],1,'S1 SENSE'), setGcf(.5)
imagesc3d2(S2 , s(S2)/2+[0 0 0], 2, 180+[0,0,0], [0,7e-3],1,'S2 SENSE'), setGcf(.5)
imagesc3d2(b1 , s(b1)/2+[5 0 0], 3, 180+[0,0,0], [0,1.5],1,'Pulseq AFI b1 SENSE'), setGcf(.5), colormap jet
