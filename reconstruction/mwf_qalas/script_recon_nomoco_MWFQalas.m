...
for t = 1:s(img_rsos, 4)
    imagesc3d2( img_rsos(:,:,:,t), s(img_rsos)/2, t, [0,180,180], [0,3e-4]), setGcf(.5)
end
...
imagesc3d2( rsos(img_patref_pad,4), s(img_patref_pad)/2, 10, [0,180,180], [0,2e-4])
...
for t = 1:size(kspace,5)
    ...
    for n = 1:num_outer_iters
        ...
        imagesc3d2(res, mtx_size/2, t, [0,0,0], [0,1e-4], 0, ['CS recon iter: ', num2str(n)]), setGcf(.5)
    end
...
for t = 1:size(img_sense,4)
    nexttile;
    imshow(abs(sq(img_cs(end/2,:,:,t))),[0,3e-4]);
end
